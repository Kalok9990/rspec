...
describe 'working through various arrays' do
  ...
  context 'test multidimensional arrays' do
    ...
    it 'the bloodhound should be 6 years old and healthy' do
      # Use a mix of array methods and indexes
      dogs = dog_breed_names_age_and_health.select{|dog| dog.first == "bloodhound"}
      p dogs
      # dogs = []
      #
      # dog_breed_names_age_and_health.each do |dog|
      #   if dog.first == "bloodhound"
      #     dogs.push(dog)
      #   end
      # end

      expect(dogs.first.last[0]).to eq 6
      expect(dogs.first.last[-1]).to eq "healthy"
    ...
    it 'the first dog should be a jack russell and 4 years old' do
      ...
      expect(dog_breed_names_age_and_health[0][-1][-1]).to eq "healthy"
    end
